...
class WaterSamples < ActiveRecord::Base
  ...
  def self.find(sample_id)
    with_scope(:find => {:conditions => "id = #{sample_id}"}) do
      #super will raise exception
      #first will hide the exception and return nil
      first
    end
  end
  ...
  # different filtration units to remove a specific Trihalomethane (but the city 
  ...
  def factor(factor_weights_id)
    if FactorWeights.exists? factor_weights_id
      sample = FactorWeights.find(factor_weights_id)
      chemicals = []
      weights = []
      chemicals << self.chloroform << self.bromoform << self.bromodichloromethane << self.dibromichloromethane

      weights << sample.chloroform_weight << sample.bromoform_weight << sample.bromodichloromethane_weight << sample.dibromichloromethane_weight

      #elegent but hard to read
      #chemicals.zip(weights).inject(0) { |sum, (a,b)| sum + a * b }

      #optimal performance
      sum, i, size = 0, 0, chemicals.size
      while i < size
        sum += chemicals[i] * weights[i]
        i += 1
      end
      sum
    else
      puts "Factor weight ID #{factor_weights_id} does not exist in the database."
    end
  end
  ...
  def to_hash(include_factors = false)
    if !self.nil?
      hash = {}
      self.attributes.each { |k,v| hash[k.to_sym] = v }
      if include_factors
        FactorWeights.all.each do |weight|
          hash["factor_#{weight.id}".to_sym] = self.factor(weight.id)
        end
      end
      hash
    else
      puts "Nothing to convert to hash."
    end
  end
end
